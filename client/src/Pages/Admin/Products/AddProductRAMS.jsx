...
export default function AddProductRAMS() {
  ...
  const [formFields, setFormFields] = useState({ productRam: "" });
  const inputRef = useRef();

  useEffect(() => {
    fetchDataFromApi("/api/productRAMS").then((res) => setProductRamData(Array.isArray(res) ? res : []));
  }, []);
  ...
  const addproductRam = (e) => {
    e.preventDefault();
    if (!formFields.productRam?.trim()) {
      setAlertBox({ open: true, error: true, msg: "Please add Product RAM" });
      return;
    }
    setIsLoading(true);
    if (!editId) {
    ...
    <div className="right-content w-100">
      <div className="card shadow border-0 w-100 flex-row p-4 mt-2">
        ...
        <Breadcrumbs aria-label="breadcrumb" className="ml-auto breadcrumbs_">
          ...
          <StyledBreadcrumb component={Link} to="/dashboard/products" label="Product RAMS" deleteIcon={<ExpandMoreIcon />} />
          <StyledBreadcrumb label="Add Product RAMS" deleteIcon={<ExpandMoreIcon />} />
        </Breadcrumbs>
      </div>
