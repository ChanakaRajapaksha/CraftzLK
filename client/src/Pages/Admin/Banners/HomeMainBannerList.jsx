...
export default function HomeMainBannerList() {
  const [slideList, setSlideList] = useState([]);
  const { setAlertBox } = useOutletContext();

  useEffect(() => {
    window.scrollTo(0, 0);
    fetchDataFromApi("/api/homeBanner").then((res) => setSlideList(Array.isArray(res) ? res : []));
  }, []);
      ...
      <div className="card shadow border-0 w-100 flex-row p-4 align-items-center">
        ...
        <div className="ml-auto d-flex align-items-center">
          <Breadcrumbs aria-label="breadcrumb" className="ml-auto breadcrumbs_">
            ...
            <StyledBreadcrumb label="Home Main Banners" deleteIcon={<ExpandMoreIcon />} />
          </Breadcrumbs>
          <Link to="/dashboard/homeBannerSlide/add">
                ...
                <tr key={item._id || item.id}>
                  ...
                  <td>
                    <div className="actions d-flex align-items-center">
                      <Link to={`/dashboard/homeBannerSlide/edit/${item._id || item.id}`}>
                        <Button className="success" color="success"><FaPencilAlt /></Button>
                      </Link>
                      <Button className="error" color="error" onClick={() => deleteSlide(item._id || item.id)}>
                        <MdDelete />
                      </Button>
                    </div>
                  </td>
