...
export default function HomeSideBannersList() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    fetchDataFromApi("/api/homeSideBanners").then((res) => setList(Array.isArray(res) ? res : []));
  }, []);
  ...
  return (
    <div className="right-content w-100">
      <div className="card shadow border-0 w-100 flex-row p-4 align-items-center">
        <h5 className="mb-0">Home Side Banners List</h5>
        <div className="ml-auto d-flex align-items-center">
          <Breadcrumbs aria-label="breadcrumb" className="ml-auto breadcrumbs_">
            <StyledBreadcrumb component={Link} to="/dashboard" label="Dashboard" icon={<HomeIcon fontSize="small" />} />
            <StyledBreadcrumb label="Home Side Banners" deleteIcon={<ExpandMoreIcon />} />
          </Breadcrumbs>
          <Link to="/dashboard/homeSideBanners/add">
                ...
                <tr key={item._id || item.id}>
                  ...
                  <td>
                    <div className="actions d-flex align-items-center">
                      <Link to={`/dashboard/homeSideBanners/edit/${item._id || item.id}`}>
