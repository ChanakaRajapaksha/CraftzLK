...
export default function SubCategoryList() {
  ...
  const parentsWithChildren = catData?.categoryList?.filter((item) => item?.children?.length > 0) || [];
    ...
    <div className="right-content w-100">
      <div className="card shadow border-0 w-100 flex-row p-4 align-items-center">
        ...
        <div className="ml-auto d-flex align-items-center">
          ...
          <Link to="/dashboard/subCategory/add">
      ...
      <div className="card shadow border-0 p-3 mt-4">
        <div className="table-responsive mt-3">
          <table className="table table-bordered table-striped v-align">
            ...
            <tbody>
              {parentsWithChildren.map((item, index) => (
                <tr key={item._id || index}>
                  ...
                  <td>
                    {item?.children?.map((subCat) => (
                      <span key={subCat._id} className="badge badge-primary mx-1">
                        {subCat.name}{" "}
                        <IoCloseSharp
                          className="cursor"
