...
export default function EditCategory() {
  ...
  useEffect(() => {
    ...
    fetchDataFromApi(`/api/category/${id}`).then((res) => {
      ...
      if (cat) {
        setFormFields({ name: cat.name || "", images: cat.images || [], color: cat.color || "" });
  ...
  const onChangeFile = (e, apiEndPoint) => {
    ...
    uploadImage(apiEndPoint, formdata).then(() => {
      fetchDataFromApi("/api/imageUpload").then((response) => {
        if (response?.length) {
          const img_arr = [];
          response.forEach((item) => item?.images?.forEach((img) => img_arr.push(img)));
  ...
  const editCat = (e) => {
    ...
    if (!payload.name.trim() || !payload.color.trim() || previews.length === 0) {
    ...
    <div className="right-content w-100">
      <div className="card shadow border-0 w-100 flex-row p-4 mt-2">
        ...
        <Breadcrumbs aria-label="breadcrumb" className="ml-auto breadcrumbs_">
          ...
          <StyledBreadcrumb label="Edit Category" deleteIcon={<ExpandMoreIcon />} />
        </Breadcrumbs>
      </div>
