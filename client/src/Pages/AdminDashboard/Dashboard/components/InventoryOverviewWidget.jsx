  ...
  return (
    <section className="admin-dash__widget admin-dash__widget--inventory">
      ...
      <div className="admin-dash__inventory-health">
        ...
        <div className="admin-dash__inventory-bar" role="img" aria-label={`Available ${available}, Low stock ${lowStock}, Out of stock ${outOfStock}`}>
          {availablePct > 0 && (
            <span className="admin-dash__inventory-bar-seg admin-dash__inventory-bar-seg--available" style={{ width: `${availablePct}%` }} />
          )}
          {lowPct > 0 && (
            <span className="admin-dash__inventory-bar-seg admin-dash__inventory-bar-seg--low" style={{ width: `${lowPct}%` }} />
          )}
          {outPct > 0 && (
            <span className="admin-dash__inventory-bar-seg admin-dash__inventory-bar-seg--out" style={{ width: `${outPct}%` }} />
          )}
        </div>
      ...
      <Link to={`${ADMIN_BASE}/products`} className="admin-dash__btn admin-dash__btn--ghost admin-dash__inventory-btn">
        Manage Inventory
      </Link>
    </section>
  );
