...
class HomeBottomBannerService {
  ...
  async list() {
    ...
    if (!bannerList) {
      const error = new Error('Failed to load bottom banners');
  ...
  async getById(id) {
    const slide = await HomeBottomBanners.findById(id);
  ...
  async create(body, uploadedImages) {
    ...
    if (!newEntry) {
      const error = new Error('Failed to create bottom banner');
      error.statusCode = 500;
      error.payload = { success: false };
      throw error;
    }
  ...
  async remove(id) {
    const item = await HomeBottomBanners.findById(id);
    ...
    const deletedItem = await HomeBottomBanners.findByIdAndDelete(id);
...
module.exports = new HomeBottomBannerService();
