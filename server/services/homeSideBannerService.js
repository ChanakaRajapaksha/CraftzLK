...
class HomeSideBannerService {
  ...
  async list() {
    ...
    if (!bannerList) {
      const error = new Error('Failed to load side banners');
  ...
  async getById(id) {
    const slide = await HomeSideBanners.findById(id);
  ...
  async create(body, uploadedImages) {
    ...
    if (!newEntry) {
      const error = new Error('Failed to create side banner');
      error.statusCode = 500;
      error.payload = { success: false };
      throw error;
    }
  ...
  async remove(id) {
    const item = await HomeSideBanners.findById(id);
    ...
    const deletedItem = await HomeSideBanners.findByIdAndDelete(id);
...
module.exports = new HomeSideBannerService();
